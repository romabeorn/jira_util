...
class TestRun(Vars):
    ...
    def download_test_run_xml_file(self):
        try:
            self.json_request_for_xml['viewSpec']['columns'][-2]['params']['testRunId'] = self.selected_test_run_id
...
class StructureIDs(Vars):
    ...
    def get_structure_order_id_by_name(self, decision):
        for id, name in enumerate(self.structure_list):
            if name == decision:
                decision = id
        return decision
    ...
    def setup_interactive_mode(self, decision):
        """ Запуск интерактивного режима """
        try:
            self.fast_order()
...
class JiraHelper:
    ...
    def main():
        ...
        if decision == '1':
            ...
            test_run_inter_active_mode.get_structure_test_runs_from_jira()
            test_run_inter_active_mode.order_gotten_test_runs()
            print(test_run_inter_active_mode.test_runs_list)
